...
var SiteMarker        = null;
var SiteCircleMarkers = [];
var Sweep             = null;

// dx = DUMP1090 Exchange indicator
var dx = {
	"|": "/",
	"/": "-",
	"-": "\\",
...
function fetchData() {
	$.getJSON('/dump1090/data.json', function(data) {
		// Store epoch timestamp (useful for call debuggin)
		LastData = Math.round( new Date().getTime() / 1000 );
		...
		for (var j=0; j < data.length; j++) {
			...
			if ( plane.funcUpdateData(data[j]) ) {
				PlanesOnMap++;
			}
		...
		// Draw a heartbeat indicator for successful data fetch
		var dxElem = $('#dx').get(0);
		dxElem.innerHTML = ( dxElem.innerHTML.length == 1 ? dx[dxElem.innerHTML] : '|' );
		dxElem.style.color = '#000'; // reset any error indicator
	}).fail(function() { $('#dx').get(0).style.color = '#F00'; } )
}

// Retrieve any home location settings from the server.  Will
// override config:{ShowSite, SiteLat, and SiteLon} if provided.
// Only called it config:SiteFromLoc is true (default)
function fetchLocation() {
	$.getJSON('/dump1090/loc.json', function(data) {
		// codeAddress calls siteShow() on success
		if ( data.home ) {
			codeAddress( data.home );
		} else if ( data.lat || data.lon ) {
			// Using the geocoder for lat/lon provides validation
			// and makes for cheap re-use.
...
function refreshSelected() {
	...
	html += '<td>ICAO (hex): ';
	if (selected) {
		html += selected.icao + " <a href=\"javascript:void( icao24Lookup( '" + selected.icao + "' ) );\">[AirFrames.org]</a>";
	} else {
		html += 'n/a'; // Something is wrong if we are here
	}
	selected.icao + '</td></tr>';
	...
	if (selected && selected.vPosition) {
		...
		if (SiteShow) {
			...
			var planeLatLon = selected.funcGLatLng();
			var dist = google.maps.geometry.spherical.computeDistanceBetween(siteLatLon, planeLatLon);
			var heading = normalizeTrack( Math.round( google.maps.geometry.spherical.computeHeading(siteLatLon, planeLatLon) ), true );
			if (Metric) {
				dist /= 1000;
			} else {
				dist /= 1852;
			...
			html += '<tr><td colspan="' + columns + '" align="center">Distance from Site: ' + dist +
				(Metric ? ' km' : ' NM') + ' ' + heading[2] + '&deg; (' + heading[1] +
				')</td></tr>';
		} // End of SiteShow
	} else {
		if (SiteShow) {
...
function normalizeTrack(track, valid){
	x = [];
	if ( !valid || isNaN(parseFloat(track)) ) {
		x = [" ", "n/a", ""];
	} else {
		track %= 360; // Bring it to within the cardinal range
		if ( track < 0 ) { // Handle negative bearings
			track += 360;
		}
		// Increased specificity and programmatic derivation
		var cardinal = {
			"N":     0,
			"NNE":  22.5,
			"NE":   45,
			...
			"NW":  315,
			"NNW": 337.5,
			"N2":  360 // Needed for planes between NNW & N
		};
		var dir = null;
		for (var point in cardinal) {
			if (
				!dir ||
				Math.abs( track - cardinal[point] ) < Math.abs( track - cardinal[dir] )
			) {
				dir = point;
			}
		}
		...
		if (!tableplane.reapable) {
			...
			if (SiteShow && (typeof SiteLat !== 'undefined' || typeof SiteLon !== 'undefined')) {
				...
				if (tableplane.vPosition) {
					var siteLatLon  = new google.maps.LatLng(SiteLat, SiteLon);
					var planeLatLon = tableplane.funcGLatLng();
					var dist = google.maps.geometry.spherical.computeDistanceBetween (siteLatLon, planeLatLon);
						if (Metric) {
							dist /= 1000;
						} else {
...
function showSite() {
	...
	if (SiteShow && (typeof SiteLat !==  'undefined' || typeof SiteLon !==  'undefined')) {
		if ( !SiteMarker ) {
			...
			SiteMarker.setPosition(new google.maps.LatLng(SiteLat, SiteLon));
		}
		GoogleMap.setCenter(SiteMarker.getPosition());
		
		// Will move to the new location automatically, so only need
		// to run this routine if we're new.
		if (SiteCircles && !SiteCircleMarkers.length) {
			for (var radius in SiteCirclesDistances) {
				SiteCircleMarkers[radius] = drawCircle(SiteMarker, radius); // in meters
			}
		}
	}
}

// Take home string (such as was fed to dump1090 on startup) and geocode it for a new map center.
function codeAddress( address ) {
	var geocoder = new google.maps.Geocoder();
	geocoder.geocode( { 'address': address}, function(results, status) {
		if (status == google.maps.GeocoderStatus.OK) {
			SiteLat  = results[0].geometry.location.lat();
			SiteLon  = results[0].geometry.location.lng();
			SiteShow = true;
			showSite();
		}
	});
...
// Open a new window with airframes.org ICAO24 lookup
function icao24Lookup( hex ) {
	document.getElementById('airframes_ica024').value = hex;
	// Necessary because we need to have the "submit" element, but that makes ".submit()"
	// function calls ambiguous and angry.
	HTMLFormElement.prototype.submit.call(document.getElementById('airframes_org'));
}

// Add an animation interval which sets or clears animation interval
// appropriately.  Assumes all projections are based on a duration of
// 60 seconds, so if you change that there you'll need to change it here.
function animateTrajectories(pause, projectionSeconds) {
	if ( !projectionSeconds ) {
		projectionSeconds = ProjectionDist;
	}
	if ( pause ) {
		window.clearInterval( AnimInterval );
	} else if ( !AnimInterval ) {
		AnimInterval = window.setInterval(function updatePlaneAnimation() {
			for ( var hex in Planes ) {
				if ( !Planes[hex].projection ) {
					continue;
				}
				Planes[hex].animCount = ( Planes[hex].animCount +  1) % ( projectionSeconds * 10 );
				Planes[hex].projection.icons[0].offset = (Planes[hex].animCount / ( projectionSeconds / 10 )) + '%';
				Planes[hex].projection.set( 'icons', Planes[hex].projection.icons );
				// TODO: Make the Sweep line to a selected plane follow its projection animation
			}
		}, 100);
	}
}
