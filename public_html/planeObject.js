var planeObject = {
	...
	funcAddToTrack: function funcAddToTrack(){
			// TODO: Write this function out
			this.trackdata.push([this.latitude, this.longitude, this.altitude, this.track, this.speed]);
			this.trackline.push(this.funcGLatLng());
		},

	// This is to remove the line from the screen if we deselect the plane
	funcClearLine: function funcClearLine() {
			if (this.line) {
				this.line.setMap(null);
				this.line = null;
			}
			// Remove the sweep line from this plane
			if (this.is_selected && Sweep) {
				Sweep.setMap( null );
			}
		},

	// This is to remove the projection line and animation
	funcClearProjection: function funcClearProjection() {
			if (this.projection) {
	...
	funcGetIcon: function funcGetIcon() {
	...
	funcSelectPlane: function funcSelectPlane(){
			selectPlaneByHex(this.icao);
		},

	// Update our data
	funcUpdateData: function funcUpdateData(data){
			// Report whether we're on the map, so to speak
			var onMap   = false;
			...
			if ((data.validposition == 1) && (this.reapable == false)) {
				...
				// Update projection lines, resetting animation if we've moved.
				if (this.vTrack && this.speed && PlaneProjection) {
					this.funcUpdateProjection( changeLat || changeLon );
				}
				onMap = true;
			} else {
	...
	funcUpdateMarker: function funcUpdateMarker() {
			if (this.marker) {
				this.marker.setPosition(this.funcGLatLng());
				this.marker.setIcon(this.funcGetIcon());
			} else {
				this.marker = new google.maps.Marker({
					position: this.funcGLatLng(),
					map: GoogleMap,
	...
	funcUpdateLines: function funcUpdateLines() {
			if (this.line) {
				var path = this.line.getPath();
				path.push(this.funcGLatLng());
			} else {
			...
			if( this.is_selected && SiteShow && SiteSweep ) {
				if ( !Sweep ) {
					Sweep = new google.maps.Polyline({
						strokeColor: SweepColor,
						strokeOpacity: 1.0,
						strokeWeight: 2,
						map: GoogleMap
					});
				}
				// Already set, but have been made invisible
				if ( Sweep.getMap() == null ) {
					Sweep.setMap( GoogleMap );
				}
				Sweep.setPath([
					new google.maps.LatLng( SiteLat, SiteLon ),
					this.funcGLatLng()
				]);
			}
			return this.line;
	...
	funcUpdateProjection: function funcUpdateProjection( resetAnim ) {
			if (this.projection) {
				this.projection.setPath([
					this.marker.getPosition(),
					this.funcProjectOffset()
				]);
				if ( resetAnim ) {
					this.animCount = 0;
				}
			} else {
				var animIcon = this.funcGetIcon();
				// Smaller, lighter.
				animIcon.scale       *= 0.5;
				animIcon.fillOpacity *= 0.75;
				animIcon.rotation = 0; // Inherited from the projection path
				this.projection = new google.maps.Polyline({            
					path: [
						this.marker.getPosition(),
						this.funcProjectOffset(),
					],
	...
	funcProjectOffset: function funcProjectOffset( duration ) {
			duration = ( duration ? duration : ProjectionDist );
			// TODO: when normalizeSpeed grows up, we'll need to
			// pass something real in the "valid" arg slot
			var distance = (
				( normalizeSpeed( this.speed, true ) * ( 1852 / 3600 ) ) // knots to ms
				* duration				   // time seconds
			);
			return google.maps.geometry.spherical.computeOffset(
				this.funcGLatLng(),
				distance, // In meters per second & duration
				this.track
			);
		},

	funcGLatLng: function funcGLatLng() {
			// TODO: Cache this value unless latitude & longitude have changed?
